...
class Config(object):
    def __init__(self, **kwargs):
        ...
        elif 'my_data' in self.kwargs['datapath']:
            self.mean      = np.array([[[63.73, 80.12, 57.11]]])
            self.std       = np.array([[[4.87, 8.35, 4.93]]])
        ...
        elif 'my_data_1100' in self.kwargs['datapath']:
            self.mean      = np.array([[[61.42, 77.18,55.03]]])
            self.std       = np.array([[[4.47,6.85, 4.36]]])
        else:
            #raise ValueError
...
class Data(Dataset):
    def __init__(self, cfg):
        with open(os.path.join(cfg.datapath , cfg.mode+'.txt'), 'r') as lines:
            self.samples = []
            for line in lines:
                imagepath = os.path.join(cfg.datapath, 'image', line.strip() + '.png')
