...
class Aggregator(nn.Module):
    def __init__(self, 
        ...
        self.rotation_module = RotationInvariantModule(128, 8)
        self.class_module = ClassRelationModule(128, 16)
    ...
    def conv_decoder(self, x, guidance):
        B = x.shape[
            0]  # print(x.shape, guidance[0].shape, guidance[1].shape) #torch.Size([3, 128, 4, 24, 24]) torch.Size([3, 32, 48, 48]) torch.Size([3, 16, 96, 96])
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        # torch.Size([4, 32, 48, 48]) torch.Size([4, 16, 96, 96]) torch.Size([4, 8, 192, 192])
        ...
        x = self.rotation_module(corr_embed)
        # 再进行类别维度的类别关系建模
        corr_embed = self.class_module(x)
