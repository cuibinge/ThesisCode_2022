...
GID3_CATEGORIES = [
    ...
    {"color": [0, 0, 255], "id": 5, "name": "Water", "trainId": 2},
]


def _get_coco_stuff_meta():
    stuff_ids = [k["id"] for k in GID3_CATEGORIES]


    stuff_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(stuff_ids)}
    stuff_classes = [k["name"] for k in GID3_CATEGORIES]
    stuff_color = [k["color"] for k in GID3_CATEGORIES]
...
def register_GID3(root):
    root = os.path.join(root, "GID3")
