...
from src.utils.misc.logger_tool import Logger
...
class CDEvaluator():
    ...
    def __init__(self, args, dataloader):
        ...
        # define logger file
        logger_path = os.path.join(args.vis_dir, 'log_test.txt')
    ...
    def _collect_running_batch_states(self):
        ...
        if np.mod(self.batch_id, 1) == 0:
            ...
            # 确保预测图为 uint8 类型
            vis_pred = np.clip(vis_pred * 255, 0, 255).astype(np.uint8)
            # 创建单独保存预测图的文件夹路径
            pred_dir = os.path.join(self.vis_dir, 'predictions')
            os.makedirs(pred_dir, exist_ok=True)


            # 保存预测图
            pred_file_name = os.path.join(pred_dir,str(self.batch_id+1) + '.png')
