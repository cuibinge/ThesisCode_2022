...
"""Base config for Boundary Attention with UNet backbone."""
...
from src.models.helpers import get_input_opts
...
def get_config(
    model_part: str = 'boundary_attention',
    ...
    """Returns base config for Boundary Attention with UNet backbone."""
    ...
    config.experiment_name = f"{model_part}_{dataset_name}_{training_type}_{time_now}"

    # Infra.
    config.eval_during_train = False  # does eval on the train device
    config.disable_pmap_and_jit = False
    config.visualize = True

    # Dataset.
    config.dataset = dataset_configs.get_dataset_config(dataset_name, dataset_dir, input_size)
    config.batch_size = config.dataset.get('train_batchsize', 1)
    config.eval_batch_size = config.dataset.get('eval_batchsize', 1)

    # Model.
    config.model = model_configs.get_model_config(model_part)
    ...
    config.init_from.checkpoint_path = config.model.get('checkpoint_path', checkpoint_path)
    config.init_from.checkpoint_step = config.model.get('checkpoint_step', checkpoint_step)
    config.init_from.params_path = config.model.get('pretrained_params_path', weights_path)
