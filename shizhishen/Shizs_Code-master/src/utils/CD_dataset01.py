...
from src.utils.data_utils import CDDataAugmentation
...
SEG_FOLDER_A = "label_A"
SEG_FOLDER_B = "label_B"
IGNORE = 255
...
def get_labA_path(root_dir, img_name):
    return os.path.join(root_dir, SEG_FOLDER_A, img_name)

def get_labB_path(root_dir, img_name):
    return os.path.join(root_dir, SEG_FOLDER_B, img_name)
def get_cdlabel_path(root_dir, img_name):
...
#        print(A_path)
        img = np.asarray(Image.open(A_path).convert('RGB'))
        img_B = np.asarray(Image.open(B_path).convert('RGB'))
        L_path = get_cdlabel_path(self.root_dir, self.img_name_list[index % self.A_size])
        L_path_A = get_labA_path(self.root_dir, self.img_name_list[index % self.A_size])
        L_path_B = get_labB_path(self.root_dir, self.img_name_list[index % self.A_size])

        label = np.array(Image.open(L_path), dtype=np.uint8)
        L_A = np.array(Image.open(L_path_A), dtype=np.uint8)
        L_B = np.array(Image.open(L_path_B), dtype=np.uint8)
        #  二分类中，前景标注为255
        ...
        [img, img_B], [label, L_A, L_B] = self.augm.transform([img, img_B], [label, L_A, L_B], to_tensor=self.to_tensor)
        # print(label.max())
        return {'name': name, 'A': img, 'B': img_B, 'L': label, 'L_A':L_A, 'L_B':L_B}
