...
import src.utils.data_config
from src.utils.CD_dataset import CDDataset
from src.utils.CD_dataset01 import CDDataset as CDDataset01
from src.utils import *

def get_loader(data_name, img_size=256, batch_size=8, split='test',
               is_train=False, is_Val = False, dataset='CDDataset'):
    ...
    if dataset == 'CDDataset':
        data_set = CDDataset(root_dir=root_dir, split=split,
                                 img_size=img_size, is_train=is_train,is_Val=is_Val,
...
def get_loaders(args):
    ...
    if args.dataset == 'CDDataset':
        ...
        val_set = CDDataset(root_dir=root_dir, split=split_val,
                                 ...
                                 label_transform=label_transform)
    elif args.dataset == 'CDDataset01':
        training_set = CDDataset01(root_dir=root_dir, split=split,
                                 img_size=args.img_size,is_train=True,
                                 label_transform=label_transform)
        val_set = CDDataset01(root_dir=root_dir, split=split_val,
                                 img_size=args.img_size,is_train=False,
                                 label_transform=label_transform)
    else:
        raise NotImplementedError(
