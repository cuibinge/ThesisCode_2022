...
class CDAM2(nn.Module):
    ...
    def __init__(self, k_size=9):
        ...
        self.conv1 = nn.Conv1d(16, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.conv2 = nn.Conv1d(16, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
...
class CDAM4(nn.Module):
    ...
    def __init__(self, k_size=5):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.relu1 = nn.ReLU()
        self.conv1 = nn.Conv1d(8, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.conv2 = nn.Conv1d(8, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.conv11 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.conv22 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.sigmoid = nn.Sigmoid()
        self.convout = nn.Conv2d(64 * 4 * 5, 64 * 5, kernel_size=3, padding=1, bias=False)
...
class RCFSNet(nn.Module):
    def __init__(self, num_classes=1, ccm=True, norm_layer=nn.BatchNorm2d, is_training=True, expansion=2,
        ...
        # 假设 firstconv 是你的模型中的卷积层
        self.firstconv1 = nn.Conv2d(in_channels=4, out_channels=64, kernel_size=7, stride=2, padding=3)

        self.firstconv = self.firstconv1
...
if __name__ == "__main__":
    ...
    #a = np.random.rand(1, 4, 128, 128)
    a = np.random.rand(1, 4, 64, 64)
