...
def _work(process_id, model, dataset, args):
    ...
    refine_classifier.load_state_dict(
        torch.load(osp.join(args.procam_weight_dir, 'refine_classifier_' + str(args.procam_num_epoches) + '.pth')))
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        if args.par_refine:
            ...
            par = par.cuda()
#             varm = VARM(num_iter=20, dilations=[1, 4, 16, 32, 48, 64])
#             varm = varm.cuda()
        ...
        for iter, pack in enumerate(data_loader):
            ...
            # outputs = [model.forward1(img[0].cuda(non_blocking=True), refine_classifier.classifier.weight) for img in
            #            pack['img']]  # b x 2 x w x h
            ...
            original_image = (original_image - original_image.min()) / (
                        original_image.max() - original_image.min())  # 归一化
            ...
            # 保存叠加后的图像
            cam_image_path = f'/chenruipeng/weakly/WRTNet/WRTNet/data/workspace/procam_2025_2/{img_name}_procam_overlay.png'
...
def run(args):
    ...
    model.load_state_dict(
        torch.load(osp.join(args.procam_weight_dir, 'res50_procam_' + str(args.procam_num_epoches) + '.pth')))
    model.eval()
