...
class DiceLoss(nn.Module):
    ...
    def forward(self, output, target):
        # Apply softmax to the output to obtain probabilities
        output = F.softmax(output, dim=1)
        
        # Convert target to one-hot encoding
        target = F.one_hot(target.squeeze(1), num_classes=output.size(1))  # Shape: [B, H, W, C]
        target = target.permute(0, 3, 1, 2).type(torch.cuda.FloatTensor)   # Shape: [B, C, H, W]
        
        # Flatten output and target for Dice coefficient calculation
