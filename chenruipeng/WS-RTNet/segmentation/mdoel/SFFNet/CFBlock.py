...
class CFBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.down = Conv(in_channels, out_channels, kernel_size=3, stride=2, dilation=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.down(x)
        x1 = x
        x = x_res + self.drop_path(self.attn_l(x_res))
        x = x1 + self.drop_path(self.mlp_l(x1)) 
        return x
    
if __name__ == '__main__':
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    input=torch.randn(2,32,256,256).to(device)
    print(input.shape)
