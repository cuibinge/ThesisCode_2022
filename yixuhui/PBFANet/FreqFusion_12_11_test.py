...
class FreqFusion(nn.Module):
    def __init__(self,
        ...
        self.lr_channel_compressor = nn.Conv2d(lr_channels * 2, self.compressed_channels, 1)
        ...
        if self.hamming_window:
            ...
            print('hamming_lowpass',
                  torch.FloatTensor(hamming2D(lowpass_kernel + 2 * lowpass_pad, lowpass_kernel + 2 * lowpass_pad))[
                      None, None,])
            ...
            print('hamming_highpass',
                  torch.FloatTensor(hamming2D(highpass_kernel + 2 * highpass_pad, highpass_kernel + 2 * highpass_pad))[
                      None, None,])
        else:
        ...
        if self.semi_conv:
            if self.comp_feat_upsample:
                if self.use_high_pass:
                    ...
                    compressed_hr_feat = carafe(compressed_hr_feat, mask_hr_init, self.highpass_kernel, self.up_group,
                                                1)
